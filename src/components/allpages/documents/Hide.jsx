...
const Hide = ({ open, setOpen, selectedItems, clearCheckbox }) => {
  ...
  useEffect(() => {
    if (selectedItems?.length) {
      const data = selectedItems.map((item) => {
        const moduleName = `${item?.ModuleName}Name`;
        const moduleID = `${item?.ModuleName}ID`;
        const name = item?.ContentName || item[moduleName];
        return {
          itemName: name,
          name: item?.ModuleName || item?.ModuleMaster?.ModuleName || null,
          ModuleTypeID: item?.ModuleTypeID || null,
          data: item[moduleID] || item?.ContentID || [],
        };
      });
      ...
      data.forEach((item) => {
        initialCheckedItems[item.data] = true; // Check all by default
      });
  ...
  const onConfirmHandler = async ({
    ...
    try {
      // Filter only the selected items based on checkedItems
          ...
          <>
            ...
            <Box className="d-flex">
                  ...
                  >
                    {filteredElements.map((content, index) => (
                      <div className="assing-list" key={index}>
                        ...
                        <Checkbox
                          color="primary"
                          checked={checkedItems[content.data] || false} // Bind checkbox state to the corresponding item in checkedItems using `data`
                          onChange={() => handleCheckChange(content.data)} // Handle check/uncheck action
                        />
                      </div>
                    ))}
                  </Box>
                  ...
                  >
                    ...
                    >
                      <Typography
                        ...
                        sx={{ mt: 2, mb: 1, fontWeight: 600 }}
                      >
                        Choose Due Date <span style={{ color: "red" }}>*</span>
                      </Typography>
                      <TextField
                        type="date"
              ...
              <div>
                <FormGroup>
                  ...
                  <FormControl>
                    ...
                    {errors.departmentOrRole && (
                      <FormHelperText>
                        At least one department or role must be selected
                      </FormHelperText>
                    )}
                    <Box
                      sx={{
                ...
                <Box>
                  <Typography>Auditors </Typography>
                  <Autocomplete
                    multiple
                ...
                <Box>
                  <Typography>Signatory </Typography>
                  <Autocomplete
                    multiple
