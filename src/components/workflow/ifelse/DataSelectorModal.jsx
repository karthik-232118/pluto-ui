...
export default function DataSelectorModal({
  ...
  const handleAdd = (item, key) => {
    onAdd(item, targetIndex, key);
    onClose();
    // Immediately add the item to the parent
  };
      ...
      for (const el of parentIds) {
        for (const [k, v] of Object.entries(nodeData?.data)) {
          if (el?.toString() === k.toString()) {
            ...
            } else if (v.type === "Concatenation") {
              list.push({
                title: v?.title,
                id: k,
                childs: [{ label: "Concatenate Value" }],
              });
            } else if (v.type === "Find") {
              list.push({
                title: v?.title,
                id: k,
                childs: [{ label: "Find Value" }],
              });
            } else if (v.type === "Remove HTML") {
              list.push({
                title: v?.title,
                id: k,
                childs: [{ label: "Remove HTML Value" }],
              });
            } else if (v.type === "Replace") {
              list.push({
                title: v?.title,
                id: k,
                childs: [{ label: "Replace Value" }],
              });
            } else if (v.type === "Split") {
              list.push({
                title: v?.title,
                id: k,
                childs: [{ label: "Split Value" }],
              });
              // } else if (v.type === "Gmail") {
              //   list.push({
              //     title: v?.title,
              //     id: k,
              //     childs: [],
              //   });
            } else if (v.type === "Form API") {
              list.push({
                title: v?.title,
                id: k,
                childs: [],
              });
            } else if (v.type === "Recieve HTTP") {
              list.push({
                title: v?.title,
                id: k,
                childs: [],
              });
            } else if (v.type === "HTTP Call") {
              list.push({
                title: v?.title,
                id: k,
                childs: [],
  ...
  return (
    <Modal open={open && listData.length} onClose={onClose}>
      <Box
        sx={{
          position: "absolute",
          top: "50%",
          ...
          <>
            ...
            <Collapse in={isExpanded[i]}>
              ...
              >
                {item?.childs?.map((el) => (
                  <ListItem key={el.label} disableGutters>
                    ...
                    <Button
                      ...
                      size="small"
                      onClick={() =>
                        handleAdd(`${item?.id}.${el?.label}`, targetKey)
                      }
                    >
                      Add
                    </Button>
                ...
                <Divider sx={{ my: 1 }} />
                <ListItem key={item?.id + 'common'} disableGutters>
                  <Typography variant="body2" sx={{ flexGrow: 1, color: 'grey' }}>
                  ...
                  <Button
                    ...
                    size="small"
                    onClick={() =>
                      handleAdd(`${item?.id}.NodeExcutionSatus`, targetKey)
                    }
                  >
                    Add
                  </Button>
